...
public abstract class AggregatedUninstallModulesMetadata {
  ...
  private static AggregatedUninstallModulesMetadata create(
    ...
    XAnnotation annotation = element.getAnnotation(ClassNames.AGGREGATED_UNINSTALL_MODULES);

    return new AutoValue_AggregatedUninstallModulesMetadata(
        element,
        env.requireTypeElement(annotation.getAnnotationValue("test").asString()),
        annotation.getAsStringList("uninstallModules").stream()
            .map(it -> env.requireTypeElement(it))
