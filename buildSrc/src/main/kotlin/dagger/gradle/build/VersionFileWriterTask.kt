...
fun Project.configureVersionFileWriter(
  ...
  val versionWriterTask = tasks.register<VersionFileWriterTask>(TASK_NAME)

  afterEvaluate { configureVersionFile(versionWriterTask, daggerExtension) }
...
fun Project.configureVersionFileWriter(
  ...
  resources.srcDir(versionWriterTask.map { it.outputDir })
...
private fun configureVersionFile(
  versionWriterTask: TaskProvider<VersionFileWriterTask>,
  daggerExtension: DaggerBuildExtension,
) {
  versionWriterTask.configure {
    ...
    this.relativePath.set("META-INF/${group}_${artifactId}.version")
